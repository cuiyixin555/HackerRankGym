...
// How to build: g++ -std=c++17 -Wall -o Day2_Operators Day2_Operators.cpp
// How to run: ./Day2_Operators

void solve(double meal_cost, int tip_percent, int tax_percent) {
  double tips = meal_cost * (static_cast<float>(tip_percent) / 100);
  double tax = meal_cost * (static_cast<float>(tax_percent) / 100);
  double total_cost = meal_cost + tips + tax;
  int32_t output = std::round(total_cost);
